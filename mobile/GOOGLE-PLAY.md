...
Prepared September 25, 2026. Google Play distribution is requested but is not live yet. The initial scope is internal testing, not a public production launch. Internal testing supports up to 100 selected testers and Play Store app updates.
...
Verified September 25: account verification gates cleared and contact phone shows verified. The owner explicitly approved the Developer Program Policies, Play App Signing Terms and US export declaration. HunterOS was created with English (United States), App, Free and the available existing package. Play app ID: 4972291825989850904. No public release or internal rollout has been submitted. No ID/address documents or payment credentials were copied into project records. Do not repeat registration/payment.

Google generated its default signing identity when the app was created. It is not the EAS certificate used by existing family APKs. The actual App signing preferences dialog supports Export and upload a key from Java keystore using Google's PEPK encrypted ZIP. That form is prepared, but no signing change or private-key transfer is complete. Owner confirmation of the encrypted existing-key transfer was requested. Public encryption key and official PEPK tool were downloaded; no private signing data was downloaded or uploaded.

After compatible signing is configured, upload the finished bundle to internal testing. Each family tester needs their own Google/Workspace account, with that exact address on the tester list, and must opt in through the actual generated test link. Android family Google addresses have been requested and are still required; do not use the iPhone tester's Hotmail address as an assumed Android login. The owner can join for setup, but that does not authorize making family members account administrators.
