...
function database(){return db??=(async()=>{const d=await SQLite.openDatabaseAsync('hunteros-mobile-v1.db');await d.execAsync('PRAGMA journal_mode=WAL; CREATE TABLE IF NOT EXISTS workspace (id INTEGER PRIMARY KEY CHECK(id=1), json TEXT NOT NULL);');return d;})().catch(error=>{db=null;throw error;});}
