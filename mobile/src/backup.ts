...
export async function exportFile(json:string,kind:'backup'|'recovery'='backup'){if(!FS.cacheDirectory||!await Sharing.isAvailableAsync())throw Error('File sharing is unavailable.');const file=FS.cacheDirectory+`HunterOS-${kind}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;await FS.writeAsStringAsync(file,json);await Sharing.shareAsync(file,{mimeType:'application/json',UTI:'public.json'});}
