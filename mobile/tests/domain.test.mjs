...
test('expanded catalog preserves every original configuration without duplicate IDs',()=>{const originals=JSON.parse(readFileSync(new URL('../data/catalog-source.json',import.meta.url),'utf8'));assert.equal(originals.length,131);assert.ok(originals.every(original=>products.some(product=>product.id===original.id)));assert.equal(new Set(products.map(p=>p.id)).size,products.length);});
...
test('favorites filter does not pollute ordinary searches',()=>{const a=filterProducts(products,{favorites:[]});assert.equal(a.length,0);assert.equal(filterProducts(products,{}).length,products.length);});
...
test('rejects unsupported schemas without mutating caller state',()=>{const s=state();assert.throws(()=>validateWorkspace({...s,version:99}));assert.equal(s.version,2);});
